...
class ContourletCNN(nn.Module):
    ...
    def __pdfbdec(self, x, method="resize"):
        ...
        # Obtain coefficients
        coefs = batch_multi_channel_pdfbdec(x=x, pfilt="9-7", dfilt="vk", nlevs=self.n_levs, device=self.device)
    ...
    def forward(self, x, y):
        ...
        x = torch.cat((upsample(x), y), dim=1)
...
def Net(args):
    return ContourletCNN(input_dim=(5, 64, 64), n_levs=[0, 3, 3, 3], num_classes=args['Categories_Number'], variant="SSF",
                          spec_type="all").to(args['device'])


if __name__ == '__main__':
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # x = Image.open('images/Lenna.png').convert("RGB")
    # x = cv2.imread('images/Lenna.png')
    # x = to_tensor(x).transpose((2, 1, 0))
    # x = np.expand_dims(x[:4, :64, :64], axis=0)
    # x = torch.from_numpy(x).type(torch.FloatTensor).to(device)

    x = torch.randn((20, 4, 16, 16)).to(device)
    y = torch.randn((20, 1, 64, 64)).to(device)
    model = ContourletCNN(input_dim=(5, 64, 64), n_levs=[0, 2, 2, 2], num_classes=10, variant="SSF",
