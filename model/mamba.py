...
class VisionMamba(nn.Module):
    def __init__(self,img_size=224,patch_size=16,stride=16,depth=24,embed_dim=192,channels=3,num_classes=1000,ssm_cfg=None,
                 drop_rate=0.,drop_path_rate=0.1,norm_epsilon: float = 1e-5,rms_norm: bool = False,initializer_cfg=None,
                 fused_add_norm=False,residual_in_fp32=False,device=None,dtype=None,ft_seq_len=None,pt_hw_seq_len=14,
                 if_bidirectional=False,final_pool_type='none',if_abs_pos_embed=False,if_rope=False,if_rope_residual=False,
                 flip_img_sequences_ratio=-1.,if_bimamba=False,bimamba_type="none",if_cls_token=False,if_devide_out=False,
    ...
    def forward(self, ms, pan, return_features=False, inference_params=None, if_random_cls_token_position=False,
                if_random_token_rank=False):
        upsample = nn.Upsample(scale_factor=4, mode='bilinear')
        m = upsample(ms)
        x = torch.concat((m, pan), dim=1)
...
def Net(args):
    return VisionMamba(img_size=64,
                       channels=5,
                       num_classes=args['Categories_Number'],
                       patch_size=8, stride=4, embed_dim=92, depth=24, rms_norm=True, residual_in_fp32=True,
                       fused_add_norm=True,
