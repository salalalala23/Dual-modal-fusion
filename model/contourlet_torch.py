...
class ContourDec(torch.nn.Module):
    def __init__(self, nlevs, device='cuda'):
        super().__init__()
        self.nlevs = nlevs
        self.device = device

    def forward(self, x):
        return pdfbdec_layer(x, self.nlevs, device=self.device)
...
class ContourRec(torch.nn.Module):
    ...
    def forward(self, x):
        return pdfbrec_layer(x[0], x[1], self.nlevs)
...
def pdfbdec_layer(x, nlevs, pfilt=None, dfilt=None, device='cuda'):
    # Laplacian decomposition
    xlo, xhi = lpdec_layer(x, device=device)
    # print(xlo.shape, xhi.shape)
    if nlevs != 0:
        # Use the ladder structure (whihc is much more efficient)
        xhi = dfbdec_layer(xhi, nlevs, device=device)
    ...
    return xlo.to(x.device), xhi.to(x.device)
...
### TODO DFB
def dfbdec_layer(x, n, device='cuda'):
...
def pdfbrec_layer(xlo, xhi, nlevs, pfilt=None, dfilt=None):
    xh = []
    ch = int(xhi.shape[1] / (2 ** nlevs))
    for i in range(2 ** nlevs):
        xh.append(xhi[:, i * ch:(i + 1) * ch, :, :])
        k = xh[i].shape
        if i < 2 ** nlevs // 2:
            trans = transforms.Compose([transforms.Resize((int(k[2] // 2), k[3]))])
        else:
            trans = transforms.Compose([transforms.Resize((k[2], int(k[3] // 2)))])
        xh[i] = trans(xh[i])

    xh = dfbrec_layer(xh)

    x = lprec_layer(xlo, xh)
    return x
...
def test():
    img = cv2.imread('I:/matlab/barbara.png')
    print(img.shape)
    # img = np.mean(img, axis=2)
    # img = np.expand_dims(img, axis=0)
    # img = np.concatenate([img] * 5, axis=0)
    img = np.expand_dims(np.mean(img, axis=2), axis=0)
    ...
    print(xlo.shape, xhi.shape, xhi.max())
    print(time() - tic)
    import matplotlib.pyplot as plt
    ...
    for i in range(5):
        ...
        for i in range(4):
            for j in range(4):
                ...
                # plt.title("ms d {} ".format(i * 8 + j + 1))
                mix = torch.sqrt(xhi[0, i, :, :] * xhi[0, j, :, :])
                # mix = mix.view(1, 1, mix.shape[0], mix.shape[1])
                # pooling = torch.nn.Upsample(mode='bilinear',scale_factor=4)
                # mix = pooling(mix)
                # cmap = sns.cubehelix_palette(start=0, rot=10, gamma=2, as_cmap=True)
                sns.heatmap(mix, cmap="rainbow", annot=False, cbar=False)  # cmap可以选择其他颜色映射，annot为True会显示数值标签
    ...
    #             plt.subplot(4, 4, i * 4 + j + 1)
    #             # plt.title("ms d {} ".format(i * 8 + j + 1))
    #             mix = (xhi[0, i, :, :]*xhi[0, j, :, :]).detach().numpy()
    #             qi = xlo[0, 0, :, :].detach().numpy()
    #
    #             ori = cv2.normalize(qi, None, 0, 255, cv2.NORM_MINMAX)
    #             ori = np.asarray(ori, dtype=np.uint8)
    #             ori = np.expand_dims(ori, axis=2)
    #             ori = np.concatenate([ori, ori, ori], axis=2)
    #             masks = cv2.normalize(mix, None, 0, 255, cv2.NORM_MINMAX)
    #             mask = np.asarray(masks, dtype=np.uint8)
    #
    #             mask = cv2.applyColorMap(mask, cv2.COLORMAP_JET)
    #             ori = cv2.addWeighted(ori, 0.7, mask, 0.3, 0)
    #
    #             plt.imshow(ori)
    #             # mix = mix.view(1, 1, mix.shape[0], mix.shape[1])
    #             # pooling = torch.nn.Upsample(mode='bilinear',scale_factor=4)
    ...
    # for i in range(3):
    #     xlo, xhi = ContourDec(nlevs)(img1)
    #     for i in range(4):
    #         for j in range(4):
    #             plt.subplot(4, 4, i * 4 + j + 1)
    #             # plt.title("ms d {} ".format(i * 8 + j + 1))
    #             mix = torch.log(xhi[0, i, :, :] * xhi[0, j, :, :])
    #             plt.title("")
    #             plt.hist(mix, bins=20, alpha=0.9, label=i)
    #             plt.tick_params(axis='both', which='both', bottom=False, top=False, left=False, right=False)  # 隐藏刻度线
    #             plt.xticks([])  # 隐藏x轴刻度标签
    #             plt.yticks([])
    #             # plt.xlabel("Columns")
    ...
    #     plt.show()
    #     img1 = xlo
    # fig, axs = plt.subplots(nrows=2, ncols=4, figsize=(256, 256))
    # axs = axs.flatten()
    # for j, ax in enumerate(axs):
    #     a = torch.mul(xhi[0, 0:3, :, :],xhi[0, j*3:(j+1)*3, :, :])
    #     print(a.shape)
    #     ax.imshow(a.permute(1, 2, 0), cmap="gray")
    #     ax.axis('off')
    # plt.show()
...
def test1():
    ...
    # img = torch.from_numpy(img).type(torch.FloatTensor)
    device = 'cuda'
    img = torch.randn([200, 4, 32, 32]).to(device)
    nlevs = 2
    t1 = time()
    pri = ContourDec(nlevs, device)(img)
    pri = ContourDec(nlevs, device)(img)
    pri = ContourDec(nlevs, device)(img)
    pri = ContourDec(nlevs, device)(img)
    pri = ContourDec(nlevs, device)(img)
...
# res = ContourRec(nlevs)(pri)
# mse = torch.mean((img - res) ** 2)
# print(mse)
